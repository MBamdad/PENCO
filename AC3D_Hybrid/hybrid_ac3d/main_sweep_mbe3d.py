...
from functions import (
    ...
    physics_collocation_tau_L2_MBE,
    energy_penalty_mbe,
    physics_guided_update_mbe_optimal,
    mass_penalty,
)
...
# Save dirs (MBE3D)
MODELS_DIR = Path("./MBE3d_models")
MAT_DIR    = Path("./MBE3d_mat")
FAST_SAVE  = True
...
def _collect_frames_and_volumes(model, mat_path, test_id, frames, T_in, Nt, downsample=1):
    ...
    return (
        _asF32F(np.stack(pred_slices, axis=0)),
        _asF32F(np.stack(exact_slices, axis=0)),
        _asF32F(np.stack(pred_vols,   axis=0)),
...
def train_fno_hybrid_LOGGING_STEPBASED(model, train_loader, test_loader, optimizer, scheduler, device, pde_weight=None):
    ...
    assert CFG.PROBLEM == 'MBE3D', "This sweep is intended for MBE3D."

    CLIP_NORM = 1.0
    logs = {k: [] for k in [
        'epoch', 'data_loss', 'phys_loss', 'energy_loss', 'scheme_loss',
        'total_loss', 'test_relL2', 'l_mid_norm_mbe', 'lr'
    ]}
    ...
    print("Epoch |   Time   | DataLoss | PhysLoss | TotalLoss | Test relL2 | energy_loss | scheme_loss | l_mid_norm_mbe | LR")
    ...
    for ep in range(CFG.EPOCHS):
        ...
        logs['l_mid_norm_mbe'].append(l_mid_norm_acc/n_batches)
        logs['lr'].append(lr)
...
def main():
    assert CFG.PROBLEM == 'MBE3D', f"config.PROBLEM must be 'MBE3D' (got {CFG.PROBLEM})"
    ...
    for ntrain in DEFAULT_N_TRAINS:
        ...
        # we keep fixed test split via build_loaders & N_TEST_FIXED
    ...
    mat_name = f"MBE3D_sweep_results_{int(time.time())}.mat"
