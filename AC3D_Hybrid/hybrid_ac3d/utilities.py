...
# ---------------------
# Hybrid trainer (PINNs-style debug prints)
# ---------------------
        ...
        for x, y in train_loader:
            ...
            if amp_allowed:
                with autocast('cuda', enabled=True):
                    ...
                    l_mid = physics_residual_midpoint(u_in_last, y_pred)
...
def evaluate_stats_and_plot(model, mat_path, test_ids, times):
    ...
    with h5py.File(mat_path, "r") as f:
        ...
        pred = rollout_autoregressive(model, gt, config.T_IN_CHANNELS, Nt=config.TOTAL_TIME_STEPS)

        # stats across test ids
        for sid in test_ids:
            gt_raw = np.array(dset[:, :, :, :, sid], dtype=np.float32)
            gt_s = np.transpose(gt_raw, (3,2,1,0))
            pred_s = rollout_autoregressive(model, gt_s, config.T_IN_CHANNELS, Nt=config.TOTAL_TIME_STEPS)
            for t in times:
                rel_errors[t].append( relative_l2_scalar(pred_s[t], gt_s[t]) )
