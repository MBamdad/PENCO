...
from functions import (
    # core physics
    scheme_residual_fourier,          # AC uses explicit residual path here
    energy_penalty,                   # AC/CH energy hinge
    semi_implicit_step,               # AC teacher is inside this generic function
    physics_collocation_tau_L2_AC,    # AC Gauss–Lobatto L2 collocation
    physics_guided_update_ac_optimal, # AC PGU (semi-implicit residual LS)
    low_k_mse,                        # spectral anchor
)
...
# --------------------
# TRAIN LOOP: AC path with step-based training + logging
# (math identical to utilities AC branch)
# --------------------
def train_fno_hybrid_LOGGING_STEPBASED(model, train_loader, test_loader, optimizer, scheduler, device, pde_weight=None):
    ...
    assert CFG.PROBLEM == 'AC3D', "This sweep is intended for AC3D."
    ...
    print("Epoch |   Time   | DataLoss | PhysLoss | TotalLoss | Test relL2 | energy_loss | scheme_loss | l_mid_norm_ac | LR")
    ...
    for ep in range(CFG.EPOCHS):
        ...
        logs['l_mid_norm_ac'].append(l_mid_norm_acc/n_batches)
        logs['lr'].append(lr)
