...
os.environ['CUDA_VISIBLE_DEVICES'] = cf.gpu_number
...
a_ind = inp[cf.index, :, :, :]
plot_field_history(cf.domain, [a_ind], ['Initial Value'], [0], problem)

u_pred = pred[cf.index, :, :, :]
u_exact = exact[cf.index, :, :, :]
fields = [u_pred, u_exact, u_pred-u_exact]
field_names = ['Exact Value', 'Predicted Value', 'Error']
plot_field_history(cf.domain, fields, field_names, cf.time_steps, problem)

make_video(u_pred, cf.domain, problem)
